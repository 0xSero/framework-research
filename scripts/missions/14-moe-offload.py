...
"""Mission 14: -ncmoe sweep for Qwen3.5-122B (MoE CPU offload)."""
...
def run(n: int):
    cmd = (
        f"timeout 500s bash -c '{BENCH} -m {MODEL} -ngl 99 -ncmoe {n} -fa 1 "
...
def main():
    ...
    entry = {
        "mission": "14-moe-offload",
        "status": "complete",
        "date": "2026-04-16",
        "primary_metric": best["pp"] if best else 0,
        "primary_unit": "best_pp_t_s",
        "conclusion": (
            f"Best ncmoe={best['ncmoe']}: pp={best['pp']} tg={best['tg']}"
